...
namespace CollegeSchedule.Api.Mihalev.Services
{
    public class ScheduleService : IScheduleService
    {
        ...
        private async Task<List<Schedule>> LoadSchedules(int groupId, DateTime start, DateTime end)
        {
            return await _db.Schedules
                ...
                .Include(s => s.Classroom)
                    .ThenInclude(c => c.Building)  // Здесь ThenInclude работает, потому что мы внутри Include
        ...
        private static LessonDto BuildLessonDto(IGrouping<dynamic, Schedule> lessonGroup)
        {
            ...
            // Добавляем заглушку для FULL, если ее нет
        ...
        private static ScheduleByDateDto BuildEmptyDayDto(DateTime date)
        {
            ...
            {
                LessonDate = date,
                Weekday = date.ToString("dddd"), // Локализованное имя дня
